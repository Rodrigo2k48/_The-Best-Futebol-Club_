...
import {
  ...
  NEW_MATCHE_OUTPUT,
  ROWS_UPDATED,
  TEAM_COMPETING_WITH_ITSELF,
} from '../integration/mocks/matches';
...
describe('GET /matches', () => {
  ...
  describe('em caso de sucesso', () => {
    it('deve retornar a lista com todas as partidas e enviar status 200', async () => {
      sinon.stub(Model, 'findAll').resolves(MATCHES_IN_DB);
      const response = await chai.request(app).get('/matches');

      expect(response.status).to.equal(HTTP_STATUS.SuccessOK);
...
describe('GET /matches?InProgress', () => {
  ...
  describe('em caso de sucesso', () => {
    ...
    it('deve retornar a lista das partidas que estão em andamento e enviar status 200', async () => {
      ...
      expect(body[0]).to.have.property('awayTeam').property('teamName').equal('Internacional');
    });
    it('deve retornar a lista das partidas que já estão finalizadas e enviar status 200', async () => {
      ...
      expect(body[0]).to.have.property('awayTeam').property('teamName').equal('Grêmio');
    });
  });
});
  ...
  describe('em caso de erro', () => {
    it('deve retornar uma mensagem de erro caso tente cadastrar um time competindo com ele mesmo e enviar status 422', async () => {
      ...
      const { status, body } = await chai
        ...
        .post('/matches')
        .set({ Authorization: TOKEN_VALID })
        .send(TEAM_COMPETING_WITH_ITSELF);
    ...
    it('deve retornar uma mensagem de erro caso o usuário tente castrar um time que não esteja registrado no banco de dados e enviar status 404', async () => {
      sinon.stub(Model, 'findAll').resolves(MATCHES_IN_DB);
      sinon.stub(Model, 'create').resolves(NEW_MATCHE_OUTPUT);
      sinon.stub(ValidateMatch.prototype, 'checkIfMatchDuplicate').resolves(false);
...
describe('PATCH /matches/id/finish', () => {
  ...
  describe('em caso de sucesso', () => {
    it('deve retornar uma mensagem afirmando a atualização para dar fim da partida e enviar o status 200', async () => {
      sinon.stub(Model, 'update').resolves([ROWS_UPDATED]);
      const { status, body } = await chai.request(app).patch('/matches/1/finish').set({ Authorization: TOKEN_VALID });
      expect(status).to.equal(HTTP_STATUS.SuccessOK);
  ...
  describe('em caso de erro', () => {
    it('deve retornar uma mensagem de erro caso o token estiver invalido e enviar status 401', async () => {
      const { status, body } = await chai.request(app).patch('/matches/2/finish').set({ Authorization: TOKEN_INVALID });
    ...
    it('deve retonar uma mensagem de erro caso o usuário não estiver com um token e enviar status 401', (done) => {
      chai
        .request(app)
...
describe('PATCH /matches/id', () => {
  ...
  describe('em caso de erro', () => {
    it('deve retornar uma mensagem de erro caso o token estiver inválido e enviar status 401', async () => {
      const { status, body } = await chai.request(app).patch('/matches/2').set({ Authorization: TOKEN_INVALID });
      expect(status).to.equal(HTTP_STATUS.ClientErrorUnauthorized);
      expect(body).to.have.property('message').equal('Token must be a valid token');
    });
    it('deve retonar uma mensagem de erro caso o usuário não estiver com um token e enviar status 401', (done) => {
