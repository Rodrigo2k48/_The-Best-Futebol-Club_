...
import {
  ONE_VALID_USER_IN_DB,
  TOKEN_INVALID,
  TOKEN_VALID,
  USER_EMAIL_REXEG_INVALID,
  USER_NOT_IN_DB,
  USER_PASS_LENGTH_PASSWORD_INVALID,
  USER_VALID,
} from '../integration/mocks/login';
...
describe('POST /login', async () => {
  ...
  describe('em caso de sucesso', () => {
    it('deve retornar um token para o usuário e enviar status 200', async () => {
  ...
  describe('em caso de erro por falta de informações', () => {
    ...
    it('(validação de Regex) deve retornar uma mensagem de erro caso o email não estiver no formato invalido enviar status 401', async () => {
      ...
      const { status, unauthorized, body } = await chai.request(app).post('/login').send(USER_EMAIL_REXEG_INVALID);
    ...
    it('deve retornar uma mensagem de erro caso o "password" tiver menos de 6 caracteres e enviar status 401', async () => {
      sinon.stub(AuthService.prototype, 'getUserInDb').resolves(ONE_VALID_USER_IN_DB[0]);
      const { status, unauthorized, body } = await chai
        .request(app)
        .post('/login')
  ...
  describe('caso o usuário não esteja cadastrado no banco de dados', () => {
    it('deve retornar uma mensagem de erro e enviar status 401', async () => {
      ...
      const { status, unauthorized, body } = await chai.request(app).post('/login').send(USER_NOT_IN_DB);
...
describe('GET /login/role', () => {
  ...
  describe('em caso de sucesso', () => {
    it('deve retornar a role do usuário logado na aplicação e enviar status 200', async () => {
      sinon.stub(Model, 'findAll').resolves(ONE_VALID_USER_IN_DB);
  ...
  describe('em caso de erro', () => {
    ...
    it('deve retornar uma mensagem de erro caso o usuário não estiver com um token e enviar um status 401', (done) => {
      chai
        .request(app)
        .get('/login/role')
        .end((err, res) => {
